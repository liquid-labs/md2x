...
1. **Fix Title Precedence For Per-File Conversions** (`001-fix-title-precedence-for-batch-conversions.md`, tier `sonnet-high`) — Fixed followup CwaE: src/cli/md2x.sh's per-file conversion loop no longer unconditionally overwrites TITLE from each input's basename — --title/-t is now honored whenever exactly one file will be converted outside --single-page, and a new upfront gate makes combining --title with more than one file fatal before any conversion work begins. --single-page and stdin conversions unaffected. --help and docs/md2x-spec.md updated; new title-precedence.bats added covering single-file-honored, directory-search-honored, multi-file-fatal, mixed-source, and no-title regression cases.
   Commit `86be541`, merged at `be9c70d81837d0fe258fb7bdfde6d59a3d7d7bdf`.

### Phase 02 — Documentation Updates

1. **Update Architecture Docs** (`001-update-architecture-docs.md`, tier `sonnet-high`) — Architect-tier review task, no edits required. Confirmed docs/md2x-spec.md's -t/--title API table row and CLI exit-behavior sentence (both already updated by phase 01) accurately state the single-file-only precedence and multi-file fatal-error case, verified directly against phase 01's Status outcome notes and the shipped src/cli/md2x.sh code. Also confirmed UC1/UC3/UC4 and the header/footer bullet still read correctly, and confirmed docs/architecture.md needs no change - its two 'title' grep hits are unrelated design-level mechanisms (PostScript overlay diagram label, TOC-heading placement), not per-file title-precedence, which lives entirely in md2x.sh's CLI option layer outside architecture.md's scope. Recorded findings in the task doc's new Status section; no source or doc edits were needed.
   Commit `3ae2a3f`, merged at `13b1d015c610a1226c08c7044f0d57db19f749dc`.
...
- **`IcMM`** — **TOC marker: no spec use case added** — Judgment call surfaced for visibility (not requiring action): decided not to add a new use case or Constraints/assumptions entry in docs/md2x-spec.md for the marker, since the rewritten General-features bullet already documents it behaviorally. A reviewer could reasonably prefer an explicit constraints entry instead.

- **`Qa0V`** — **README --title/-t row has stale wording** — Surfaced during title-precedence planning (2026-07-31): README.md's CLI reference table carries the same stale "-t/--title ... used for the output filename and the PDF header text" wording that docs/md2x-spec.md's API table had (now being corrected there by plan/title-precedence to document the single-file-only precedence and the multi-file fatal-error case). README.md was deliberately left out of that plan's scope. Worth a small followup to bring README.md's row in line once the spec wording lands.
...
- [x] [001-fix-title-precedence-for-batch-conversions.md](./phase-01-title-precedence/001-fix-title-precedence-for-batch-conversions.md) — tier `sonnet-high` · branch `phase-01-task-01-fix-title-precedence-for-batch` · commit `86be541` · merge `be9c70d81837d0fe258fb7bdfde6d59a3d7d7bdf`

### Phase 02 — Documentation Updates

- [x] [001-update-architecture-docs.md](./phase-02-doc-updates/001-update-architecture-docs.md) — tier `sonnet-high` · branch `plan/title-precedence-02-001` · commit `3ae2a3f` · merge `13b1d015c610a1226c08c7044f0d57db19f749dc`
