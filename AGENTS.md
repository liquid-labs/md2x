...
- `src/cli/lib/` — CLI library scripts pulled into the rollup: `index.sh`, `parameters.sh` (argument parsing), `generate-page.sh` (Pandoc/Ghostscript/pdftk conversion pipeline), `toc-preprocess.py` (the Markdown-in/Markdown-out table-of-contents preprocessing stage `generate-page()` runs ahead of Pandoc), `ensure-weasyprint.sh` (bootstraps the `~/.md2x/venv` WeasyPrint install used as Pandoc's `--pdf-engine`), and `github.css` (the built-in stylesheet).
